...
FROM employee
WHERE position IN ('Seller', 'Consultant')
   OR position LIKE '%Man%'
ORDER BY employment_date DESC;
...
SELECT employee_id, last_name, first_name, position,
CASE
  WHEN position = 'Seller' THEN 'Can sell'
  WHEN position = 'Manager' THEN 'Can manage and sell'
  WHEN position = 'HR' THEN 'No direct sales'
  ELSE 'Service roles'
END AS relation_to_customer
FROM employee
ORDER BY last_name ASC;
...
-- homework
-- Список усіх клієнтів, відсортований за прізвищем

SELECT *
FROM customer
ORDER BY last_name ASC;

-- Унікальні виробники товарів (по алфавіту)

SELECT DISTINCT manufacture
FROM product
ORDER BY manufacture ASC;

-- Товари виробника DELL (коротка інформація)
...
-- Жінки, народжені з 1990 по 2000 рік
...
-- Товари типу Laptop або Desktop, що є в наявності

SELECT *
FROM product
WHERE amount > 0
  AND (product_name LIKE '%Laptop%'
       OR product_name LIKE '%Desktop%');
       
-- Інвойси без клієнта (неавторизовані покупці)
